    ...
    <div className="min-h-screen bg-[#F8F9FA] font-sans text-slate-800 pb-20 relative">
      ...
      <div className="bg-white/90 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            <h1 className="text-2xl font-black text-gray-900 cursor-pointer" onClick={() => setBusqueda('')}>
              Tienda <span className="text-emerald-600">Alada</span>
            </h1>
            <button onClick={() => setCarritoAbierto(true)} className="relative p-3 bg-gray-900 text-white rounded-xl hover:bg-emerald-600 transition shadow-lg">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {categorias.map(cat => (
              <button key={cat} onClick={() => setCategoriaActiva(cat)} className={`px-4 py-1.5 rounded-full text-sm font-bold whitespace-nowrap border transition-all ${categoriaActiva === cat ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-500 border-gray-200'}`}>{cat}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {productosProcesados.map((prod) => {
            ...
            return (
              <div key={prod._id} onClick={() => setProductoSeleccionado(prod)} className={`group bg-white rounded-2xl p-3 border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col relative ${cantidadLlevada > 0 ? 'border-emerald-500 ring-1 ring-emerald-500' : esOferta ? 'border-red-100' : 'border-gray-100'}`}>
                ...
                <div className="aspect-square rounded-xl overflow-hidden bg-gray-50 relative mb-4">
                  <img src={prod.fotoUrl} alt={prod.nombre} className="w-full h-full object-cover mix-blend-multiply group-hover:scale-105 transition-transform duration-500"/>
                  {cantidadLlevada > 0 && (
                    <div className="absolute top-2 right-2 bg-emerald-600 text-white font-black text-xs px-2 py-1 rounded-md shadow-lg flex items-center gap-1"><ShoppingBag size={10} /> {cantidadLlevada}</div>
                  )}
                  {prod.stock <= 0 && <div className="absolute inset-0 bg-white/60 flex items-center justify-center font-bold text-slate-800 backdrop-blur-[2px]">AGOTADO</div>}
                  {!prod.stock <= 0 && (
                      <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0">
                        <span className="bg-white/90 backdrop-blur p-2 rounded-full shadow-sm text-gray-700 block hover:bg-emerald-500 hover:text-white transition-colors"><Info size={16}/></span>
                      </div>
                  )}
                </div>

                <div className="flex-1 flex flex-col">
                  <h3 className="font-bold text-gray-800 leading-tight mb-1 line-clamp-2 group-hover:text-emerald-700 transition-colors">{prod.nombre}</h3>
                  <div className="mt-auto pt-3 flex items-center justify-between">
                    <div className="flex flex-col">
                      {esOferta && <span className="text-[10px] text-gray-400 line-through font-bold">${prod.precioOriginal}</span>}
                      <span className={`text-xl font-black ${esOferta ? 'text-red-600' : 'text-gray-900'}`}>${prod.precio}</span>
                    </div>
                    {prod.stock > 0 && (
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col drawer-entrada">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-white shadow-sm z-10">
              ...
              <button onClick={() => setCarritoAbierto(false)} className="p-2 bg-gray-100 hover:bg-red-100 hover:text-red-500 rounded-full transition-colors"><X size={20}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-50">
              {carrito.length === 0 ? <p className="text-center py-20 opacity-50">Tu carrito está vacío</p> : carrito.map((item) => (
                <div key={item._id} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex gap-3 items-center">
                  <img src={item.fotoUrl} className="w-16 h-16 rounded-lg object-cover bg-gray-100" alt=""/>
                  <div className="flex-1">
                    <h4 className="font-bold text-sm text-gray-800 line-clamp-1">{item.nombre}</h4>
                    <div className="flex items-center justify-between mt-2">
                        <p className="text-emerald-700 font-bold">${item.precio * item.cantidad}</p>
                        <div className="flex items-center gap-3 bg-gray-100 rounded-lg px-2 py-1">
                          <button onClick={() => modificarCantidad(item._id, -1)} className="text-gray-500 hover:text-red-500 font-bold px-1"><Minus size={14}/></button>
                  ...
                  <button onClick={() => eliminarDelCarrito(item._id)} className="p-2 text-gray-300 hover:text-red-500 transition"><Trash2 size={16}/></button>
                </div>
              ))}
            </div>
            {carrito.length > 0 && (
              <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-4px_15px_rgba(0,0,0,0.05)] z-10">
                <div className="flex justify-between items-end mb-4"><span className="text-gray-500 font-medium">Total Estimado</span><span className="text-3xl font-black text-gray-900">${totalCarrito}</span></div>
                <a 
      ...
      {productoSeleccionado && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onClick={() => setProductoSeleccionado(null)}></div>
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden relative z-10 modal-animada flex flex-col md:flex-row max-h-[85vh]">
            ...
            <div className="w-full md:w-2/5 bg-gray-50 h-64 md:h-auto relative flex items-center justify-center p-6">
               <img src={productoSeleccionado.fotoUrl} className="w-full h-full object-contain mix-blend-multiply" alt=""/>
               {productoSeleccionado.precioOriginal > productoSeleccionado.precio && (
            ...
            <div className="w-full md:w-3/5 p-8 flex flex-col overflow-y-auto">
                 ...
                 <div className="mb-6 border-b border-gray-100 pb-4">
                    {productoSeleccionado.precioOriginal > productoSeleccionado.precio ? (
                      <div className="flex items-center gap-3">
                          <span className="text-2xl font-bold text-gray-300 line-through">${productoSeleccionado.precioOriginal}</span>
                          <span className="text-4xl font-black text-red-600">${productoSeleccionado.precio}</span>
                      </div>
                    ) : (
                      <div className="text-3xl font-bold text-emerald-600">${productoSeleccionado.precio}</div>
                    )}
                 </div>
                 
                 <h4 className="font-bold text-gray-800 mb-2 text-sm">Descripción:</h4>
                 <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-line mb-6">
                    {productoSeleccionado.descripcion || "Descripción detallada no disponible."}
                 </p>
                 
                 <div className="flex items-center gap-2 text-sm text-gray-500 mb-8 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <div className={`w-2 h-2 rounded-full ${productoSeleccionado.stock > 0 ? 'bg-emerald-500' : 'bg-red-400'}`}></div>
                    Status: <span className="font-semibold text-gray-700">{productoSeleccionado.stock > 0 ? `En Stock (${productoSeleccionado.stock})` : 'Agotado'}</span>
                 </div>
               </div>
               ...
               <div className="flex flex-col gap-3 pt-4 border-t border-gray-100">
                 {productoSeleccionado.stock > 0 ? (
                   ...
                   <button disabled className="w-full bg-gray-100 text-gray-400 py-3.5 rounded-xl font-bold cursor-not-allowed">Producto Agotado</button>
                 )}
               </div>
            </div>
          </div>
